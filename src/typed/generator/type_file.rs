...
impl DefinitionWriter<'_> {
    ...
    pub fn write<W: std::io::Write>(&self, mut buffer: W) -> mlua::Result<()> {
        ...
        for definition in self.definition.iter() {
            match &definition.ty {
                Type::Value(ty) => {
                    if let Some(docs) = accumulate_docs(&[definition.doc.as_deref()]) {
                        writeln!(buffer, "{}", docs.join("\n"))?;
                    }

                    writeln!(buffer, "--- @type {}", type_signature(ty)?)?;
                    writeln!(buffer, "{} = nil", definition.name)?;
                }
                Type::Class(type_data) => {
                    if let Some(docs) =
                        accumulate_docs(&[definition.doc.as_deref(), type_data.type_doc.as_deref()])
                    {
                        writeln!(buffer, "{}", docs.join("\n"))?;
                    }
                    ...
                    for (name, field) in type_data.static_fields.iter() {
                        if let Some(docs) = accumulate_docs(&[field.doc.as_deref()]) {
                            writeln!(buffer, "{}", docs.join("\n"))?;
                        }
                        writeln!(
                            buffer,
                    ...
                    if !type_data.functions.is_empty() || !type_data.methods.is_empty() || !type_data.is_meta_empty() {
                        ...
                        for (name, func) in type_data.functions.iter() {
                            if let Some(docs) = accumulate_docs(&[func.doc.as_deref()]) {
                                writeln!(buffer, "  {}", docs.join("\n  "))?;
                            }
                            writeln!(
                                buffer,
                        ...
                        if !type_data.is_meta_empty() {
                                    ...
                                    || !type_data.meta_methods.is_empty()
                            {
                                ...
                                for (name, field) in type_data.meta_fields.iter() {
                                    ...
                                    writeln!(buffer, "--- @type {}", type_signature(&field.ty)?)?;
                ...
                Type::Module(module) => {
                    if let Some(docs) =
                        accumulate_docs(&[definition.doc.as_deref(), module.doc.as_deref()])
                    {
                        writeln!(buffer, "{}", docs.join("\n"))?;
                    }
                    ...
                    write_module(&mut buffer, module, &mut path)?;
                    writeln!(buffer)?;
                },
                other => {
...
fn function_signature<S: std::fmt::Display>(
    ...
    for (i, param) in params.iter().enumerate() {
        ...
        result.push(match param.name.as_deref() {
            Some(name) => format!("--- @param {name} {} {doc}", type_signature(&param.ty)?),
            None => format!("--- @param param{i} {} {doc}", type_signature(&param.ty)?),
        });
    }

    for ret in returns.iter() {
        let doc = ret.doc.as_deref().unwrap_or_default();
        result.push(format!("--- @return {} {doc}", type_signature(&ret.ty)?));
    }
    ...
    Ok(match ty {
        ...
        Type::Table(entries) => {
            format!(
                ...
                entries
                .iter()
                .map(|(k, v)| { Ok(format!("{k}: {}", type_signature(v)?)) })
                .collect::<mlua::Result<Vec<_>>>()?
                .join(", ")
            )
        }
...
fn write_module<B: std::io::Write>(buffer: &mut B, module: &TypedModuleBuilder, path: &mut Vec<String>) -> mlua::Result<()> {
    ...
    for (name, field) in module.fields.iter() {
        ...
        match &field.ty {
            &Type::Module(ref module) => {
                ...
                write_module(buffer, module, path)?;
                path.pop();
                writeln!(buffer, ",")?;
            },
            other => {
                writeln!(buffer, "{single_offset}--- @type {}", type_signature(other)?)?;
    ...
    for (name, nested) in module.nested_modules.iter() {
        if let Some(docs) = accumulate_docs(&[nested.doc.as_deref()]) {
            writeln!(buffer, "{single_offset}{}", docs.join(format!("\n{single_offset}").as_str()))?;
        }

        write!(buffer, "{single_offset}{name} = ")?;
        path.push(name.to_string());
        write_module(buffer, nested, path)?;
        path.pop();
        writeln!(buffer, ",")?;
    }
    ...
    for (name, func) in module.functions.iter() {
        ...
        writeln!(buffer, "{single_offset}{},", function_signature(name, &func.params, &func.returns, true)?.join(format!("\n{single_offset}").as_str()))?;
    }
    ...
    for (name, func) in module.methods.iter() {
        ...
        writeln!(buffer, "{single_offset}{},", method_signature(name, "table".into(), &func.params, &func.returns, true)?.join(format!("\n{single_offset}").as_str()))?;
    }
    ...
    if !module.is_meta_empty() {
        ...
        for (name, field) in module.meta_fields.iter() {
            ...
            match &field.ty {
                ...
                other => {
                    writeln!(buffer, "{double_offset}--- @type {}", type_signature(other)?)?;
        ...
        for (name, func) in module.meta_functions.iter() {
            ...
            writeln!(buffer, "{double_offset}{},", function_signature(name, &func.params, &func.returns, true)?.join(format!("\n{double_offset}").as_str()))?;
        }
        ...
        for (name, func) in module.meta_methods.iter() {
            ...
            writeln!(buffer, "{double_offset}{},", method_signature(name, "table".into(), &func.params, &func.returns, true)?.join(format!("\n{double_offset}").as_str()))?;
        }
